...
# Custom extensions to QuPath

This repository shall aggregate all relevant QuPath extensions and modifications. Briefly, this shall include:

* Instructions for building QuPath from source, including the following extras
* Modification of ListView; enforcing sorting of annotations and hierarchies by various criteria (RoiType, numerical order/alphabetical order)
* Custom QuPath startup package
	* Startup driver to set customized values for
		* PathClasses required for a given annotation task
		* PathPrefs amenable to a given annotation task
		* Key bindings corresponding to given classification and subclassification values
		* OverlayOptions amenable to multi-layered workflows including alignment overlays, partial display of annotations for inter-rater reliability tests, etc.
	* and load scripts to manage
		* PathClass context menus and hotkeys
		* TileManager to allow annotating and tracking subregions according to density and ratio of PathClasses per subregion
		* TPSTool-compatible clipboard
* ThinPlateSpline backend extension
* TPSTool; ThinPlateSpline driver script and tool interface
